...
// Birthdays returns everyone whose birthday falls in the next `days`, with the
// year shifted to the upcoming occurrence.
func (s *Store) Birthdays(ctx context.Context, days int) ([]Person, error) {
	rows, err := s.pool.Query(ctx, `select `+personCols+`
		from contacts c
		left join clients cl on cl.id = c.client_id and cl.deleted_at is null
		where c.deleted_at is null and c.birthday is not null
		  and (
		    (make_date(extract(year from current_date)::int,
		     ...
		     between current_date and current_date + make_interval(days => $1))
		    or
		...
		order by extract(month from c.birthday), extract(day from c.birthday)`, days)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
