...
type Song struct {
	...
	// A recording to play it against. A chart says which chords; it cannot
	// say the feel, or which of four arrangements this one is.
...
func (s *Store) CreateSong(ctx context.Context, in SongInput, actor string) (Song, error) {
	song, err := scanSong(s.pool.QueryRow(ctx, `
		insert into songs (title, artist, song_key, tempo, part, body, notes,
		                   reference_url, created_by, updated_by)
		values ($1, $2, $3, $4, $5, $6, $7, $8, $9, $9)
		returning `+songCols,
		in.Title, in.Artist, in.Key, in.Tempo, in.Part, in.Body, in.Notes,
		in.ReferenceURL, actor))
	return song, norm(err)
}
...
func (s *Store) UpdateSong(ctx context.Context, id int64, in SongInput, actor string) (Song, error) {
	song, err := scanSong(s.pool.QueryRow(ctx, `
		update songs set title = $1, artist = $2, song_key = $3, tempo = $4,
		       part = $5, body = $6, notes = $7, reference_url = $8,
		       updated_by = $9, updated_at = now()
		 where id = $10 and deleted_at is null
		returning `+songCols,
		in.Title, in.Artist, in.Key, in.Tempo, in.Part, in.Body, in.Notes,
		in.ReferenceURL, actor, id))
	return song, norm(err)
}
